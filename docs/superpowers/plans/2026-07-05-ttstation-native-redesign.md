...
- [ ] **Step 3: `DeviceStripView.swift`** — takes a `TelemetryService`; renders `service.snapshot?.devices` as a row/grid of compact per-device tiles: `dev<i>`, a temp value in `TTTheme.mono` colored via `TTTheme.tempColor`, a small horizontal temp meter, and — from the REAL `tt-smi -s` fields confirmed against the live QB2 — a power reading (`powerW`, W) and/or aiclk (`aiclkMHz`, MHz) as the "working hard" signal when present (there is NO utilization field in the frame). Shows "telemetry unavailable" (secondary) when `state == .failed`/no snapshot. A trailing `Open tt-toplike ↗` button calling the launcher. `.task`/`.onDisappear` start/stop the service for `box.record.host` / `ctrlPort`.
